...
<main class="container mt-4">
    ...
    <div class="card mb-4 position-relative">
        ...
        <div class="card-body">
            ...
            <h5 class="card-subtitle mb-2 text-muted">작성자: <%= dto.getAuthor() %> / 작성일: <%= postDate %></h5>
            <p class="card-text"><%= dto.getContent() %></p>
            <p class="card-text">조회수: <%= dto.getViews() %></p>
        </div>
    </div>
    ...
    <div class="text-center">
        <a href="<%=request.getContextPath()%>/commUpdate.jsp?idx=<%= idx %>&pageNum=<%= request.getParameter("pageNum") %>" class="btn btn-warning">수정</a>
        <a href="<%=request.getContextPath()%>/commDeleteAction.jsp?idx=<%= idx %>&pageNum=<%= request.getParameter("pageNum") %>" class="btn btn-danger" onclick="return confirm('정말 삭제하시겠습니까?');">삭제</a>
