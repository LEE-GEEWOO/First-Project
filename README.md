...
　Header , footer等全般的なメインページを始め各ページのCSSを担当
 ...
 - 공지사항 (お知らせ）
  ...
  보이게 설정했으며 관리자만 공지사항 글쓰기 권한 및 삭제 수정할 수 있게 구성 

 (お知らせ担当 
 
 DBとお知らせの部分の担当でServletを使用せず管理者又は一般ユーザーがLoginした際、目録ボタンだけ見える
 ように設定し、管理者だけお知らせページの作成権限及び削除・修正出来るよう形成)
...
- 메인페이지
 ...
 6. 페이지별 기능
  ...
  -반응형 웹 페이지로 구성-
   ...
   밑으로 내려갈시 지루하지 않도록 다이나믹한 화면 구성과 끝에 다다를시 위로 이동할 수 있는 Nav     를 구성 였으며 카카오톡 로고를 클릭하면 관리자와 소통할 수 있는 공간을 구성하였습니다. 
